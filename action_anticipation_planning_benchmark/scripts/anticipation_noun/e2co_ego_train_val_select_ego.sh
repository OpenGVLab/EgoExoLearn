...
testing_source=false # true | false
modality=RGB
...
#====== select dataset ======#
path_data_root=../egobridge_all_annotations/anticipation_annotation/noun/ # depend on users
path_exp_root=action-anticipation-noun/ # depend on users
path_feat_root=/mnt/petrelfs/share_data/chenguo/egobridge_final_annotations/feats/clip_features_5fps/
...
num_source=39440 # number of training data (source) 
num_target=39440 # number of training data (target)


train_source_list=$path_data_root"list_egoexo_cotraining.txt"
train_target_list=$path_data_root"list_egoexo_cotraining.txt"
val_list=$path_data_root"list_ego_val_ego_exo-feature.txt"
test_list=$path_data_root"list_ego_test_ego_exo-feature.txt"
val_source_list=$path_data_root"list_exo_val_ego_exo-feature.txt"
test_source_list=$path_data_root"list_exo_test_ego_exo-feature.txt"
NUM_CLASSES=31
...
then
	...
	echo 'testing on the target testing set'
...
then
	...
	# testing on the testing set
	echo 'testing on the source validation set'
	python anticipation_test_models.py $class_file $modality \
	$val_source_list $exp_path$modality'/'$model'.pth.tar' \
	--arch $arch --test_segments $test_segments \
	--save_scores $exp_path$modality'/scores_'$dataset_target'-'$model'-'$test_segments'seg' --save_confusion $exp_path$modality'/confusion_matrix_'$dataset_target'-'$model'-'$test_segments'seg' \
	--n_rnn 1 --rnn_cell LSTM --n_directions 1 --n_ts 5 \
	--use_attn $use_attn --n_attn $n_attn --use_attn_frame $use_attn_frame --use_bn $use_bn --share_params $share_params \
	...
	# testing on the testing set
	echo 'testing on the source setting set'
	python anticipation_test_models.py $class_file $modality \
	$test_source_list $exp_path$modality'/'$model'.pth.tar' \
	--arch $arch --test_segments $test_segments \
	--save_scores $exp_path$modality'/scores_'$dataset_target'-'$model'-'$test_segments'seg' --save_confusion $exp_path$modality'/confusion_matrix_'$dataset_target'-'$model'-'$test_segments'seg' \
	--n_rnn 1 --rnn_cell LSTM --n_directions 1 --n_ts 5 \
	--use_attn $use_attn --n_attn $n_attn --use_attn_frame $use_attn_frame --use_bn $use_bn --share_params $share_params \
