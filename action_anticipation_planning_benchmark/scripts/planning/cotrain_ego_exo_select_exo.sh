...
#====== select dataset ======#
path_data_root=../egobridge_all_annotations/planning_annotation/annotation/ # depend on users
path_exp_root=action-planning/ # depend on users
path_feat_root=/mnt/petrelfs/share_data/chenguo/egobridge_final_annotations/feats/clip_features_5fps/


num_source=4530 # number of training data (source) 
num_target=4530 # number of training data (target)


train_source_list=$path_data_root'/cotrain_ego_exo_anno_list.txt'
train_target_list=$path_data_root'/cotrain_ego_exo_anno_list.txt'
val_list=$path_data_root'/val_exo_anno_list.txt'
test_list=$path_data_root'/test_exo_anno_list.txt'

path_exp=$path_exp_root'Testexp-egoonly'
#====== select dataset ======#
...
then
	...
	#------ main command ------#
	python planning_main.py $class_file $modality $train_source_list $train_target_list $val_list --exp_path $exp_path \
...
then
	...
	python planning_test_models.py $class_file $modality \
