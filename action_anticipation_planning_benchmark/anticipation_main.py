...
from loss import *
from anticipation_opts import parser
...
def main():
    ...
    class_ids_list = [
        line.strip().split('|')[3] for line in open(args.train_source_list)
    ]
    class_id_list = []
    ...
    print('class_data_counts:', class_data_counts)
...
def validate(val_loader, model, criterion, num_class, epoch, log):
    ...
    all_pred_top5 = torch.scatter(
        torch.zeros_like(all_preds, device=all_preds.device), 1,
    ...
    recall = recall_score(all_labels, all_pred_top5, average='macro') * 100
    prec = precision_score(all_labels, all_pred_top5, average='macro') * 100
    map_macro = mean_average_precision(all_preds,all_labels) * 100
    print("map_macro: ", map_macro)
    print((
        'Testing Results: recall top5 {recall:.3f} prec top5 {prec:.3f}  Loss {loss.avg:.5f}'
        .format(recall=recall, prec=prec, loss=losses)))
