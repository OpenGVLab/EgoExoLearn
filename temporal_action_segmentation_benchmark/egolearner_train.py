...
class Trainer:
    ...
    def train(self, model, model_dir, results_dir, batch_gen_source_train,
              batch_gen_target_train, batch_gen_source_test,
              batch_gen_target_test, device, args):
        # ====== collect arguments ====== #
        ...
        batch_size_target = max(
            int(batch_gen_target_train.num_examples /
                batch_gen_source_train.num_examples * batch_size_source), 1)
        num_iter_epoch = np.ceil(batch_gen_source_train.num_examples /
                                 batch_size_source)
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            while batch_gen_source_train.has_next():
                ...
                # ====== Feed-forward data ====== #
                # prepare inputs
                input_source, label_source, mask_source = batch_gen_source_train.next_batch(
                    batch_size_source, 'source')
                input_source, label_source, mask_source = input_source.to(
                    device), label_source.to(device), mask_source.to(device)
    ...
    def ctrl_video_length(self, input_data, label, mask, ratio_length):
        ...
        id_drop = np.floor(np.linspace(0, num_frame - 1,
                                       num_frame_drop)).tolist()
