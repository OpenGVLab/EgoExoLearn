...
# === Mode Switch On/Off === #
experiment_type=ego-exo-cotraining-gazed-exo
cv_method=cotraining # cross-view method, zero-shot, domain-adaption, knowledge-distillation, cotrain
...
do
  ...
  if ($eval)
  then
      python egolearner_eval.py --path_data=$path_data --path_result=$path_result --dataset=$dataset --exp_type $experiment_type 
  fi
done
