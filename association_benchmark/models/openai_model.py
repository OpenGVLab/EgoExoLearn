...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward_part1(self, x, attn_mask=None):
        return self.attention(self.ln_1(x), attn_mask=attn_mask)

    def forward_part2(self, x):
        return self.mlp(self.ln_2(x))

    def forward(self, x: torch.Tensor, use_checkpoint=False, attn_mask=None):
        if use_checkpoint:
            x = x + checkpoint.checkpoint(self.forward_part1, x, attn_mask=attn_mask)
        else:
            x = x + self.forward_part1(x, attn_mask=attn_mask)
    ...
    def forward(self, x: torch.Tensor, use_checkpoint=False, attn_mask=None):
        if use_checkpoint:
            for i in range(self.layers):
                x = checkpoint.checkpoint(self.resblocks[i], x, attn_mask=attn_mask)
            return x
        else:
            if attn_mask is None:
